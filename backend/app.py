...
def create_app():
    ...
    if db_host and db_user and db_password and db_name:
        ...
        app.config["SQLALCHEMY_DATABASE_URI"] = url_object.render_as_string(
            hide_password=False
        )
    else:
        # Fall back to SQLite
...
def get_locations():
    ...
    for loc in locations:
        results.append({"id": loc.id, "name": loc.name, "address": loc.address})
...
def get_kitchens():
    ...
    for k in kitchens:
        ...
        for brand in k.brands:
            associated_brands.append({"id": brand.id, "name": brand.name})
...
def get_menus():
    ...
    results = []
    for m in menus:
        results.append({"id": m.id, "brand_id": m.brand_id})
    return jsonify(results)
...
def get_categories():
    ...
    results = []
    for c in categories:
        results.append({"id": c.id, "name": c.name, "menu_id": c.menu_id})
    return jsonify(results)
...
def create_category():
    ...
    return jsonify(
        {"id": category.id, "name": category.name, "menu_id": category.menu_id}
    )
