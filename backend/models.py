...
brand_kitchen_association = db.Table(
    'brand_kitchen',
    db.Column('brand_id', db.Integer, db.ForeignKey('brand.id', ondelete='CASCADE'), primary_key=True),
    db.Column('kitchen_id', db.Integer, db.ForeignKey('kitchen.id', ondelete='CASCADE'), primary_key=True)
)
...
class Location(db.Model):
    ...
    kitchens = db.relationship('Kitchen', backref='location', lazy=True, 
...
class Vendor(db.Model):
    ...
    kitchens = db.relationship('Kitchen', backref='vendor', lazy=True,
                              passive_deletes=False)
    # when vendor is deleted, cascade delete its brands
    brands = db.relationship('Brand', backref='vendor', lazy=True, 
                            cascade="all, delete-orphan")
...
class Brand(db.Model):
    ...
    menu = db.relationship('Menu', backref='brand', uselist=False, 
                          cascade="all, delete-orphan")
...
class Menu(db.Model):
    ...
    categories = db.relationship('Category', backref='menu', lazy=True, 
                                cascade="all, delete-orphan")
